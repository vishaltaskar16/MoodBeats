...
def save_base64_audio(base64_string, output_path):
    """
...
def api_record():
    """
    API endpoint for recording-based emotion detection
    """
    ...
    temp_webm = os.path.join(UPLOAD_FOLDER, "recorded.webm")
    temp_wav = os.path.join(UPLOAD_FOLDER, "recorded.wav")
    
    if not save_base64_audio(data['audio'], temp_webm):
        return jsonify({"error": "Error saving recorded audio!"}), 500
    
    try:
        # Convert to WAV for processing
        convert_to_wav(temp_webm, temp_wav)
    except Exception as e:
        return jsonify({"error": "Error converting recorded audio!"}), 500
